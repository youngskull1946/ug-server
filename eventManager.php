...
const TAGS = ['Teatro', 'Musica', 'Cine', 'Deportes', 'Danza'];
...
$default_timezone = 'America/Mexico_City'; // UTC-6
    ...
    if ($from != $dest) {
        ...
        if ($move) {
            ...
            if (fm_rcopy($from, $dest)) {
                fm_set_msg(sprintf('Copyied from <b
Oferta Cultural

All files uploaded to /var/www/html>%s</b> to <b>%s</b>', $copy, $msg_from));
            } else {
...
if (isset($_POST['upl'])) {
    ...
    if (empty($_POST["date"])){
        $dateError = "Este campo es obligatorio" ;
    } else { 
        $getDate = $_POST["date"] ; 
    }
    if (empty($_POST["time"])){
        $timeError = "Este campo es obligatorio" ;
    } else {
        $getTime = $_POST["time"] ;
    }
    if (empty($_POST["tags"])){
        $tagsError = "Este campo es obligatorio" ;
    } else {
        $getTags = $_POST["tags"] ;
    }
    
    //convert date and time to format, get extension, and modified date
    fm_clean_path($title);
    $date = date("d.m.y", strtotime($getDate)) ;
    $time = date("H.i", strtotime($getTime)) ;
    $ext = pathinfo($_FILES['upload']['name'][0])['extension'];//
    $dateModified = date('d.m.y,H.i');

    //tags
    for ($i = 0, $bit = 1; $i < count(TAGS); $i++, $bit *= 2) {
        if (in_array(TAGS[$i], $getTags)) {
            $tags |= $bit;
        }
    }

    $newName = $title .':'. $tags  .':'. $date .','. $time .':'. $dateModified .'.'. $ext ;
    $errors = 0;
    $uploads = 0;
...
print_r($_POST);
    ...
    $time = date("H.i", strtotime($getTime)) ;
    $ext = pathinfo($_FILES['upload']['name'][0])['extension'];
    $oldExt = pathinfo($path . '/' . $getFileName)['extension'];
    $dateModified = date('d.m.y,H.i');
    ...
    $newName = $title .':'. $tags  .':'. $date .','. $time .':'. $dateModified .'.'. ($ext != "" ? $ext : $oldExt) ;
    $errors = 0;
    $uploads = 0;
    $total = count($_FILES['upload']['name']);
    ...
    for ($i = 0; $i < $total; $i++) {
        $tmp_name = $_FILES['upload']['tmp_name'][$i];
        if (empty($_FILES['upload']['error'][$i])) {
            if (unlink($path . '/' . $getFileName) // Delete old file.
                && move_uploaded_file($tmp_name, $path . '/' . $newName)) {
                $uploads++;
            }
        } else {
            if (rename($path . '/' . $getFileName, $path . '/' . $newName)) {
                $uploads++;
            } else {
                $errors++;
            }
    ...
    if ($errors == 0) {
        fm_set_msg(sprintf('Renamed to <b>%s</b>', $path));
    } else {
        fm_set_msg(sprintf('Error while renaming %s', $uploads), 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['upload'])) {
    ...
    <div class="path">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <br><br>
            <p>Fecha: <input type="text" name="date" id="datepicker">
            <span class="error">* <?php echo $dateError;?></span></p>
            <p>Hora:
              <select name="time">
                ...
                <option value="01:00am">1:30am</option>
            ...
            <p>Etiquetas: <br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[0] ; ?>"><?php echo TAGS[0] ; ?><br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[1] ; ?>"><?php echo TAGS[1]; ?><br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[2] ; ?>"><?php echo TAGS[2]; ?><br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[3] ; ?>"><?php echo TAGS[3]; ?><br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[4] ; ?>"><?php echo TAGS[4]; ?><br>
              <span class="error"><?php echo $tagsError;?></span>
            </p>
            <p>
                <button class="btn"><i class="icon-apply"></i> A&ntilde;adir</button> &nbsp;
...
if (isset($_GET['rename'])) {
    ...
    // old name
    $old = $_GET['filename'];
    list($title, $tagMask, $dateEvent, $dateModified) = explode(":", $old);

    $dateTimeEvent = DateTime::createFromFormat('d.m.y,H.i', $dateEvent);
    if ($dateTimeEvent == false) {
        print("File doesn't have correct format");
    }
    $timestampEvent = $dateTimeEvent->getTimestamp() ;
    $dateFormat = date("m/d/Y h:ia", $timestampEvent) ;

    list($dateFile, $timeFile) = explode(" ", $dateFormat) ;
    $tags = [];
    for ($i = 0, $bit = 1; $i < count(TAGS); $i++, $bit *= 2) {
        if ($tagMask & $bit) {
            $tags[] = TAGS[$i];
    ...
    <div class="path">
        ...
        $( function() {
          $( "#datepicker" ).datepicker();
          $("input[type=date]").datepicker("setDate", "<?php echo $dateFile ;?>");
        } );

        </script>
        </head>
        <form action="" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="r" value="<?php echo fm_enc(FM_PATH) ?>">
            <input type="hidden" name="rne" value="1">
            <input type="hidden" name="filename" value="<?php echo $old ?>">
            <input type="file" name="upload[]"><br>
            <br>
            T&iacute;tulo: <input type="text" name="title" value="<?php echo $title ; ?>"><br><br>
            <p>Fecha: <input type="text" name="date" id="datepicker" value="<?php echo $dateFile ; ?>"></p>
            <p>Hora:
              <select name="time">
                <option value="11:00am" <?php if($timeFile == "11:00am") echo "selected='selected'";?> >11:00am</option>
                <option value="11:30am" <?php if($timeFile == "11:30am") echo "selected='selected'";?> >11:30am</option>
                <option value="12:00pm" <?php if($timeFile == "12:00am") echo "selected='selected'";?> >12:00pm</option>
                <option value="12:30pm" <?php if($timeFile == "12:30pm") echo "selected='selected'";?> >12:30pm</option>
                <option value="01:00pm" <?php if($timeFile == "01:00pm") echo "selected='selected'";?> >1:00pm</option>
                <option value="01:30pm" <?php if($timeFile == "01:30pm") echo "selected='selected'";?> >1:30pm</option>
                <option value="02:00pm" <?php if($timeFile == "02:00pm") echo "selected='selected'";?> >2:00pm</option>
                <option value="02:30pm" <?php if($timeFile == "02:30pm") echo "selected='selected'";?> >2:30pm</option>
                <option value="03:00pm" <?php if($timeFile == "03:00pm") echo "selected='selected'";?> >3:00pm</option>
                <option value="03:30pm" <?php if($timeFile == "03:30pm") echo "selected='selected'";?> >3:30pm</option>
                <option value="04:00pm" <?php if($timeFile == "04:00pm") echo "selected='selected'";?> >4:00pm</option>
                <option value="04:30pm" <?php if($timeFile == "04:30pm") echo "selected='selected'";?> >4:30pm</option>
                <option value="05:00pm" <?php if($timeFile == "05:00pm") echo "selected='selected'";?> >5:00pm</option>
                <option value="05:30pm" <?php if($timeFile == "05:30pm") echo "selected='selected'";?> >5:30pm</option>
                <option value="06:00pm" <?php if($timeFile == "06:00pm") echo "selected='selected'";?> >6:00pm</option>
                <option value="06:30pm" <?php if($timeFile == "06:30pm") echo "selected='selected'";?> >6:30pm</option>
                <option value="07:00pm" <?php if($timeFile == "07:00pm") echo "selected='selected'";?> >7:00pm</option>
                <option value="07:30pm" <?php if($timeFile == "07:30pm") echo "selected='selected'";?> >7:30pm</option>
                <option value="08:00pm" <?php if($timeFile == "08:00pm") echo "selected='selected'";?> >8:00pm</option>
                <option value="08:30pm" <?php if($timeFile == "08:30pm") echo "selected='selected'";?> >8:30pm</option>
                <option value="09:00pm" <?php if($timeFile == "09:00pm") echo "selected='selected'";?> >9:00pm</option>
                <option value="09:30pm" <?php if($timeFile == "09:30pm") echo "selected='selected'";?> >9:30pm</option>
                <option value="10:00pm" <?php if($timeFile == "10:00pm") echo "selected='selected'";?> >10:00pm</option>
                <option value="10:30pm" <?php if($timeFile == "10:30pm") echo "selected='selected'";?> >10:30pm</option>
                <option value="11:00pm" <?php if($timeFile == "11:00pm") echo "selected='selected'";?> >11:00pm</option>
                <option value="11:30pm" <?php if($timeFile == "11:30pm") echo "selected='selected'";?> >11:30pm</option>
                <option value="12:00am" <?php if($timeFile == "12:00am") echo "selected='selected'";?> >12:00am</option>
                <option value="12:30am" <?php if($timeFile == "12:30am") echo "selected='selected'";?> >12:30am</option>
                <option value="01:00am" <?php if($timeFile == "01:00am") echo "selected='selected'";?> >1:00am</option>
                <option value="01:00am" <?php if($timeFile == "01:30am") echo "selected='selected'";?> >1:30am</option>
                <option value="02:00am" <?php if($timeFile == "02:00am") echo "selected='selected'";?> >2:00am</option>
                <option value="02:30am" <?php if($timeFile == "02:30am") echo "selected='selected'";?> >2:30am</option>
                <option value="03:00am" <?php if($timeFile == "03:00am") echo "selected='selected'";?> >3:00am</option>
                <option value="03:30am" <?php if($timeFile == "03:30am") echo "selected='selected'";?> >3:30am</option>
                <option value="04:00am" <?php if($timeFile == "04:00am") echo "selected='selected'";?> >4:00am</option>
                <option value="04:30am" <?php if($timeFile == "04:30am") echo "selected='selected'";?> >4:30am</option>
                <option value="05:00am" <?php if($timeFile == "05:00am") echo "selected='selected'";?> >5:00am</option>
                <option value="05:30am" <?php if($timeFile == "05:30am") echo "selected='selected'";?> >5:30am</option>
                <option value="06:00am" <?php if($timeFile == "06:00am") echo "selected='selected'";?> >6:00am</option>
                <option value="06:30am" <?php if($timeFile == "06:30am") echo "selected='selected'";?> >6:30am</option>
                <option value="07:00am" <?php if($timeFile == "07:00am") echo "selected='selected'";?> >7:00am</option>
                <option value="07:30am" <?php if($timeFile == "07:30am") echo "selected='selected'";?> >7:30am</option>
                <option value="08:00am" <?php if($timeFile == "08:00am") echo "selected='selected'";?> >8:00am</option>
                <option value="08:30am" <?php if($timeFile == "08:30am") echo "selected='selected'";?> >8:30am</option>
                <option value="09:00am" <?php if($timeFile == "09:00am") echo "selected='selected'";?> >9:00am</option>
                <option value="09:30am" <?php if($timeFile == "09:30am") echo "selected='selected'";?> >9:30am</option>
                <option value="10:00am" <?php if($timeFile == "10:00am") echo "selected='selected'";?> >10:00am</option>
                <option value="10:30am" <?php if($timeFile == "10:30am") echo "selected='selected'";?> >10:30am</option>                
              </select>
            </p>

            <p>Etiquetas: <br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[0] ; ?>" <?php foreach ( $tags as $t ) { if ( $t == TAGS[0] ) echo 'checked'; } ?> ><?php echo TAGS[0] ; ?><br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[1] ; ?>" <?php foreach ( $tags as $t ) { if ( $t == TAGS[1] ) echo 'checked'; } ?> ><?php echo TAGS[1] ; ?><br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[2] ; ?>" <?php foreach ( $tags as $t ) { if ( $t == TAGS[2] ) echo 'checked'; } ?> ><?php echo TAGS[2] ; ?><br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[3] ; ?>" <?php foreach ( $tags as $t ) { if ( $t == TAGS[3] ) echo 'checked'; } ?> ><?php echo TAGS[3] ; ?><br>
              <input type="checkbox" name="tags[]" value="<?php echo TAGS[4] ; ?>" <?php foreach ( $tags as $t ) { if ( $t == TAGS[4] ) echo 'checked'; } ?> ><?php echo TAGS[4] ; ?><br>
            </p>
            <p>
                <button class="btn"><i class="icon-apply"></i> Editar</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> Cancelar</a></b>
            </p>
        </form>
    </div>
...
<table><tr>
<th style="width:5%"><label><input type="checkbox" title="Invert selection" onclick="checkbox_toggle()"></label></th>
<th style="width:5%">Índice</th>
<th style="width:40%">Título</th>
<th style="width:15%">Categorías</th>
<th style="width:35%">Fecha y Hora</th>
<th style="width:10%">Operaciones</th></tr>
<?php

$index = 1;
foreach ($files as $f) {
    $fileExtension = pathinfo($f, PATHINFO_EXTENSION);
    if (!in_array($fileExtension, ['jpeg', 'jpg', 'png', 'gif', 'bmp', 'tiff', 'tif'])) {
        continue;
    }
    list($title, $tagMask, $dateEvent, $dateModified) = explode(":", $f);
    $tags = [];
    for ($i = 0, $bit = 1; $i < count(TAGS); $i++, $bit *= 2) {
        if ($tagMask & $bit) {
            $tags[] = TAGS[$i];
    ...
    $dateEventString = $dateTimeEvent->format('D d M, Y, h:ia');
    ?>
<tr>
<td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td>
<td><?php echo $index ?></span></td>
<td><?php echo $title ?></span></td>
<td><?php echo implode(", ", $tags) ?></td>
<?php if (!FM_IS_WIN): ?>
<td><?php echo $dateEventString ?></td>
<?php endif; ?>
<td>
...
<a title="Rename file" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;rename&amp;filename=<?php echo $f ?>"><i class="icon-rename"></i></a>
...
<p><input type="submit" name="delete" value="Borrar eventos seleccionados" onclick="return confirm('Delete selected files and folders?')">
</form>
...
<?php if (FM_USE_AUTH): ?><a title="Logout" href="?logout=1"><i class="icon-logout"></i></a><?php endif; ?>
        ...
        echo '<div class="break-word"><h2><center>Oferta Cultural</center></h2></div>';
        ?>
</div>
<?php
}
...
<head>
<meta charset="utf-8">
<title>Oferta Cultural - Universidad de Guanajuato</title>
<style>
...
function change_checkboxe2s(l,v){for(var i=l.length-1;i>=0;i--){l[i].checked=(typeof v==='boolean')?v:!l[i].checked;}}
